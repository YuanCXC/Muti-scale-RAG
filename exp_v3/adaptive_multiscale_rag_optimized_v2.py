...
class AdaptiveMultiscaleRAGExperiment:
    ...
    def graph_expand(self, seed_titles: Sequence[str], hops: int = 1, limit_per_seed: Optional[int] = None) -> List[EvidenceUnit]:
        ...
        for title in ordered_seeds:
            # 参数化查询并统一 HTML 实体，避免 “&amp;” 与 “&” 导致种子标题匹配失败。
            cypher = f"""
            MATCH (start:Section)
            WHERE toLower(replace(start.title, '&amp;', '&')) =
            ...
            """
            try:
                rows = self.graph_store.query(cypher, {"title": title, "limit": int(limit)})
            except Exception as exc:
                logger.warning("图扩展失败（%s）：%s", title, exc)
                continue
            ...
            for row in rows:
                ...
                if new_title and content and new_title not in seen:
                    ...
                    if not path_nodes or path_nodes[0] != title:
                        path_nodes = [title] + path_nodes
    ...
    def retrieve_adaptive(
        ...
        保留“细粒度保持—父级回升—图扩展—摘要补充”的原框架，
        仅收紧图种子、邻居、二跳触发和最终候选数量。
        """
        ...
        if fine_only:
            units = unique_by_title(reranked)[: self.config.fine_max_units]
            return MethodResult(
                units=units,
        ...
        if force_parent and enable_parent:
            ...
            units = self.select_with_budget(query, parent_candidates, max_units=self.config.parent_max_units)
            return MethodResult(
                units=units,
        ...
        if route == "fine_grained":
            selected = unique_by_title(reranked)[: self.config.fine_max_units]
        elif route == "local_parent" and enable_parent:
            parent_candidates = self.parent_map(reranked)
            candidate_count = len(parent_candidates)
            selected = self.select_with_budget(query, parent_candidates, max_units=self.config.parent_max_units)
        else:
            ...
            candidate_count = len(all_candidates) + len(summaries)
            selected = self.select_with_budget(
                query,
                all_candidates + summaries,
                max_units=self.config.graph_max_units,
            )
...
If the generated answer refuses to answer or says the answer cannot be determined,
but the reference answer is specific and the retrieved context contains sufficient evidence,
set correctness to 0 and answer_relevance no higher than 0.25.
...
"""
    ...
    def run(self) -> Dict[str, Any]:
        ...
        rows_by_method = self.evaluate_methods(method_fns, desc="自适应多尺度 RAG")
        ...
        complexity_rows = self.complexity_stratified_table(proposed_rows)
        self.write_csv(run_dir / "complexity_stratified.csv", complexity_rows)

        route_rows = self.route_distribution_table(proposed_rows)
        self.write_csv(run_dir / "route_distribution.csv", route_rows)

        details = {
            "rows_by_method": rows_by_method,
            "summary": summary,
            "complexity_stratified": complexity_rows,
        ...
        logger.info("自适应多尺度 RAG 实验完成，结果目录：%s", run_dir)
        return {
            "run_dir": str(run_dir),
            "tables": {
                "result_summary": str(run_dir / "result_summary.csv"),
                "semantic_records": str(run_dir / "semantic_records.csv"),
                "complexity_stratified": str(run_dir / "complexity_stratified.csv"),
                "route_distribution": str(run_dir / "route_distribution.csv"),
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="自适应多尺度 RAG 实验（本文方法）")
