...
class RerankRAGExperiment:
    ...
    def load_resources(self) -> None:
        logger.info("加载 HotpotQA 资源（Rerank RAG）")
...
"""
    ...
    def _create_run_dir(self) -> Path:
        ...
        run_dir = self.config.output_dir / f"rerank_rag_{timestamp}"
    ...
    def run(self) -> Dict[str, Any]:
        ...
        method_fns: Dict[str, Callable[[str], MethodResult]] = {
            "Rerank RAG": self.retrieve_method,
        }
        rows_by_method = self.evaluate_methods(method_fns, desc="Rerank RAG")
            ...
            json.dump(
                {
                    "status": "completed",
                    "method": "Rerank RAG",
                    "sample_count": len(rows_by_method["Rerank RAG"]),
                },
                f,
                ensure_ascii=False,
                indent=2,
        ...
        logger.info("Rerank RAG 实验完成，结果目录：%s", run_dir)
        return {
            "run_dir": str(run_dir),
            "tables": {
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Rerank RAG 对比实验（分阶段并发）")
    ...
    result = RerankRAGExperiment(config).run()
