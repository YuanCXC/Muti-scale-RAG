...
class SemanticRAGExperiment:
    ...
    def load_resources(self) -> None:
        logger.info("加载 HotpotQA 资源（Semantic RAG）")
...
"""
    ...
    def _create_run_dir(self) -> Path:
        ...
        run_dir = self.config.output_dir / f"semantic_rag_{timestamp}"
    ...
    def run(self) -> Dict[str, Any]:
        ...
        method_fns: Dict[str, Callable[[str], MethodResult]] = {
            "Semantic RAG": self.retrieve_method,
        }
        rows_by_method = self.evaluate_methods(method_fns, desc="Semantic RAG")
            ...
            json.dump(
                {
                    "status": "completed",
                    "method": "Semantic RAG",
                    "sample_count": len(rows_by_method["Semantic RAG"]),
                },
                f,
                ensure_ascii=False,
                indent=2,
        ...
        logger.info("Semantic RAG 实验完成，结果目录：%s", run_dir)
        return {
            "run_dir": str(run_dir),
            "tables": {
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Semantic RAG 对比实验（分阶段并发）")
    ...
    result = SemanticRAGExperiment(config).run()
