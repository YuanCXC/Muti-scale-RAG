...
该脚本是 v3 评测口径下的自包含实现。检索与生成分阶段执行，
DeepSeek 活跃请求默认上限为 200。脚本不依赖其他对比实验文件。
"""
...
class RetrievalConfig:
    ...
    # 生成与评价阶段使用 DeepSeek API 高并发。检索阶段单独限流，避免
    # FAISS、Neo4j 和 API 重排序器因 200 线程同时访问而失稳。
    max_workers: int = 200
...
class KGRAGExperiment:
    ...
    def __init__(self, config: Optional[RetrievalConfig] = None):
        ...
        self.neo4j_degraded = False

        self.documents: List[Dict[str, Any]] = []
        self.title_to_content: Dict[str, str] = {}

        self._rerank_semaphore = threading.BoundedSemaphore(
    ...
    def load_resources(self) -> None:
        logger.info("加载 HotpotQA 资源（KG-RAG）")
    ...
    def _load_optional_clients(self) -> None:
        if self.config.run_generation or self.config.run_judge:
            ...
            logger.info(
                "DeepSeek 客户端就绪：generation_workers=%s, llm_concurrency=%s",
    ...
    def keyword_retrieve(self, query: str, top_k: Optional[int] = None) -> List[EvidenceUnit]:
        ...
        try:
            ...
            raise RuntimeError("关键词图检索失败") from exc
    ...
    def _get_rerank_key_lock(self, cache_key: str) -> threading.Lock:
        with self._rerank_cache_lock:
            lock = self._rerank_key_locks.get(cache_key)
            if lock is None:
                lock = threading.Lock()
                self._rerank_key_locks[cache_key] = lock
            return lock
    ...
    def rerank_units(self, query: str, units: Sequence[EvidenceUnit], top_k: Optional[int] = None) -> List[EvidenceUnit]:
        ...
        key_lock = self._get_rerank_key_lock(cache_key)
        with key_lock:
            with self._rerank_cache_lock:
                cached = self._rerank_cache.get(cache_key)
                cached_status = self._rerank_status_cache.get(cache_key)
            ...
            if not result:
                ...
                for unit in units:
                    ...
                    lexical_unit = copy.deepcopy(unit)
                    lexical_unit.score = 0.7 * unit.score + 0.3 * overlap
                    scored.append((lexical_unit.score, lexical_unit))
    ...
    def graph_expand(
        ...
        seen_titles = set(ordered_seeds)
        seen_node_ids: Set[str] = set()

        cypher = f"""
        MATCH (start:Section {{title: $title}})
        MATCH p = (start)-[:SEMANTIC_LINKS*1..{hops}]-(n:Section)
        WHERE n <> start
        ...
        for seed_title in ordered_seeds:
            ...
            for variant in self.title_variants(seed_title):
                try:
                    ...
                    raise RuntimeError(
                        f"图扩展失败（seed={seed_title}, variant={variant}）"
                    ) from exc
                if candidate_rows:
                    rows = candidate_rows
            ...
            for row in rows:
                ...
                if (
                    not new_title
                    or not content
                    or new_title in seen_titles
                    or (node_id and node_id in seen_node_ids)
                ):
                    continue
    ...
    def retrieve_method(self, query: str) -> MethodResult:
        ...
        stats["graph_hops"] = 1
        stats["neo4j_degraded"] = self.neo4j_degraded
    ...
    def _retry_delay(self, attempt: int) -> float:
        base = max(0.05, float(self.config.llm_retry_base_seconds))
        cap = max(base, float(self.config.llm_retry_max_seconds))
        jitter = max(0.0, float(self.config.llm_retry_jitter_seconds))
        return min(cap, base * (2 ** max(0, attempt - 1))) + random.uniform(0.0, jitter)

    def _deepseek_generate_once(
        self,
        ...
        *,
        ...
        ``_llm_semaphore`` 将账号层面的活跃请求控制在配置并发内。
        ThreadPoolExecutor 的线程数控制任务并发，信号量控制真正的 HTTP 并发。
        """
    ...
    def generate_answer(self, question: str, units: Sequence[EvidenceUnit]) -> Tuple[str, int, str]:
        """生成答案，返回 ``(answer, attempts, error)``。

        对空响应、429、500、503、网络超时等异常统一执行指数退避重试。
        """
        if not self.config.run_generation or self.llm_client is None:
            return "", 0, "generation_disabled"
        ...
        max_attempts = max(1, int(self.config.generation_max_retries))
        last_error = ""
        for attempt in range(1, max_attempts + 1):
            try:
                response = self._deepseek_generate_once(
                    messages,
                    temperature=0.0,
                    max_tokens=256,
                )
                content = str(getattr(response, "content", "") or "").strip()
                if bool(getattr(response, "success", False)) and content:
                    return content, attempt, ""
                last_error = str(getattr(response, "error", "") or "empty_or_unsuccessful_response")
            except Exception as exc:
            ...
            if attempt < max_attempts:
                ...
                logger.warning(
                    "DeepSeek 生成失败，第 %s/%s 次，%.2fs 后重试：%s",
                    attempt,
                    max_attempts,
                    delay,
                    last_error,
...
"""
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            if attempt < max_attempts:
                delay = self._retry_delay(attempt)
                logger.warning(
                    "DeepSeek Judge 失败，第 %s/%s 次，%.2fs 后重试：%s",
                    attempt,
                    max_attempts,
                    delay,
                    last_error,
        ...
        base["_judge_error"] = last_error if parsed is None else ""
    ...
    def evaluate_methods(self, method_fns: Dict[str, Callable[[str], MethodResult]], desc: str = "评估方法") -> Dict[str, List[Dict[str, Any]]]:
        ...
        def generation_phase(method_name: str, sample: pd.Series, method_result: MethodResult, relevant_titles: Set[str]) -> Tuple[str, Dict[str, Any]]:
            ...
            answer, generation_attempts, generation_error = self.generate_answer(
                question,
                method_result.units,
            )
        ...
        if self.config.skip_retrieval:
            ...
            retrieval_workers = min(
                total_tasks,
                max(1, int(self.config.retrieval_workers)),
            )
        ...
        if not self.config.run_generation and not self.config.run_judge:
            ...
            for method_name, sample, method_result, relevant_titles in retrieval_results:
                ...
                result = {
                    ...
                    "generation_success": False,
                    "generation_attempts": 0,
                    "generation_error": "generation_disabled",
                    "retrieval_metrics": metrics,
                    "semantic_metrics": {},
                    "stats": method_result.stats,
                    "complexity_score": route_detail.get("complexity_score", self.complexity_scorer.compute(question).score),
    ...
    def aggregate_method_rows(self, rows: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        generation_successes = sum(1 for row in rows if bool(row.get("generation_success")))
        ...
        return {
            ...
            # 各消融共享重排序缓存，该时延仅用于诊断，不用于论文横向结论。
            "Time/ms (diagnostic)": round2(mean_or_none(m.time_ms for m in metrics)) or 0.0,
    ...
    def _create_run_dir(self) -> Path:
        ...
        run_dir = self.config.output_dir / f"kg_rag_{timestamp}"
    ...
    def run(self) -> Dict[str, Any]:
        ...
        method_fns: Dict[str, Callable[[str], MethodResult]] = {
            "KG-RAG": self.retrieve_method,
        }
        rows_by_method = self.evaluate_methods(method_fns, desc="KG-RAG")

        summary = self.summarize_methods(rows_by_method)
        self.write_csv(run_dir / "result_summary.csv", summary)

        semantic_records = self.semantic_record_table(rows_by_method)
        self.write_csv(run_dir / "semantic_records.csv", semantic_records)

        type_rows = self.type_breakdown_table(rows_by_method)
        self.write_csv(run_dir / "comparison_by_type.csv", type_rows)

        details = {
            "rows_by_method": rows_by_method,
            "summary": summary,
            "semantic_records": semantic_records,
            "comparison_by_type": type_rows,
        }
        ...
        with open(config_path, "w", encoding="utf-8") as f:
            config_payload = self._json_safe(asdict(self.config))
            config_payload["neo4j_degraded"] = self.neo4j_degraded
            json.dump(config_payload, f, ensure_ascii=False, indent=2)

        checkpoint_path = self._checkpoint_path

        logger.info("KG-RAG 实验完成，结果目录：%s", run_dir)
        return {
            "run_dir": str(run_dir),
            "tables": {
                "result_summary": str(run_dir / "result_summary.csv"),
                "semantic_records": str(run_dir / "semantic_records.csv"),
                "comparison_by_type": str(run_dir / "comparison_by_type.csv"),
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="KG-RAG 对比实验（分阶段并发）")
    parser.add_argument("--sample-size", type=int, default=None, help="采样数量，默认使用配置值")
    parser.add_argument(
        "--max-workers",
        type=int,
        default=None,
        help="生成/Judge 任务线程数；默认 200",
    )
    parser.add_argument("--retrieval-workers", type=int, default=None, help="检索阶段线程数；默认 32")
    parser.add_argument("--llm-concurrency", type=int, default=None, help="DeepSeek API 活跃请求上限；默认 200")
    parser.add_argument("--rerank-concurrency", type=int, default=None, help="API 重排序并发；默认 5")
    parser.add_argument("--generation-max-retries", type=int, default=None, help="答案生成最大重试次数")
    ...
    if args.retrieval_workers is not None:
        config_kwargs["retrieval_workers"] = args.retrieval_workers
    if args.llm_concurrency is not None:
        config_kwargs["llm_max_concurrency"] = args.llm_concurrency
    if args.rerank_concurrency is not None:
        config_kwargs["rerank_max_concurrency"] = args.rerank_concurrency
    if args.generation_max_retries is not None:
        config_kwargs["generation_max_retries"] = args.generation_max_retries
