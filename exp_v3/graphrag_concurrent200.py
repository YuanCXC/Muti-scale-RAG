...
class GraphRAGExperiment:
    ...
    def load_resources(self) -> None:
        logger.info("加载 HotpotQA 资源（GraphRAG）")
    ...
    def retrieve_method(self, query: str) -> MethodResult:
        ...
        stats = self._stats(
            ...
            route="graphrag",
            candidate_count=len(seeds) + len(expanded),
        )
        stats["graph_hops"] = self.config.hmax
...
"""
    ...
    def _create_run_dir(self) -> Path:
        ...
        run_dir = self.config.output_dir / f"graphrag_{timestamp}"
    ...
    def run(self) -> Dict[str, Any]:
        ...
        method_fns: Dict[str, Callable[[str], MethodResult]] = {
            "GraphRAG": self.retrieve_method,
        }
        rows_by_method = self.evaluate_methods(method_fns, desc="GraphRAG")
            ...
            json.dump(
                {
                    "status": "completed",
                    "method": "GraphRAG",
                    "sample_count": len(rows_by_method["GraphRAG"]),
                    "neo4j_degraded": self.neo4j_degraded,
                },
                f,
                ensure_ascii=False,
                indent=2,
        ...
        logger.info("GraphRAG 实验完成，结果目录：%s", run_dir)
        return {
            "run_dir": str(run_dir),
            "tables": {
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="GraphRAG 对比实验（分阶段并发）")
    ...
    parser.add_argument("--hmax", type=int, default=None, help="固定图扩展跳数，默认 2")
    ...
    result = GraphRAGExperiment(config).run()
