...
Return JSON: {{"correctness": 0-1, "faithfulness": 0-1, "answer_relevance": 0-1, "context_relevance": 0-1}}"""
    ...
    def run(self) -> Dict[str, Any]:
        ...
        run_dir = self.config.output_dir / f"exp1_6_proposed_{timestamp}"
        run_dir.mkdir(parents=True, exist_ok=True)
        
        print("\n运行 Proposed 实验...")
        rows_by_method = self.evaluate_methods({"Proposed": self.retrieve_method}, desc="Proposed")
...
def main() -> int:
    ...
    result = ProposedExperiment(config).run()
    return 0
