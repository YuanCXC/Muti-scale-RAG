...
Return JSON: {{"correctness": 0-1, "faithfulness": 0-1, "answer_relevance": 0-1, "context_relevance": 0-1}}"""
    ...
    def retrieve_method(self, query: str) -> MethodResult:
        ...
        units = self.rerank_units(query, initial)
        elapsed = (time.time() - start) * 1000
        return MethodResult(units=units, stats={"time_ms": elapsed, "avg_len": context_len(units), "expanded_nodes": 0, "route": "rerank"})
    ...
    def run(self) -> Dict[str, Any]:
        ...
        run_dir = self.config.output_dir / f"exp1_2_rerank_rag_{timestamp}"
        run_dir.mkdir(parents=True, exist_ok=True)
        
        print("\n运行 +Rerank 实验...")
        rows_by_method = self.evaluate_methods({"+Rerank": self.retrieve_method}, desc="+Rerank")
