# -*- coding: utf-8 -*-
"""实验3.3：No summary evidence

方法：去掉摘要证据（当前实现等同于Proposed）
"""
...
class NoSummaryEvidenceExperiment:
    """No summary evidence实验"""
...
Return JSON: {{"correctness": 0-1, "faithfulness": 0-1, "answer_relevance": 0-1, "context_relevance": 0-1}}"""
    ...
    def retrieve_method(self, query: str) -> MethodResult:
        ...
        complexity = min(1.0, len(query.split()) / 15.0 + query.count("?") * 0.2)
        
        if complexity < 0.45:
            units = reranked[:self.config.k3]
            route = "fine_grained"
        ...
        return MethodResult(units=units, stats={"time_ms": elapsed, "avg_len": context_len(units), "expanded_nodes": len(units) - len(reranked[:self.config.k3]), "route": route, "complexity_score": complexity})
    ...
    def run(self) -> Dict[str, Any]:
        ...
        run_dir = self.config.output_dir / f"exp3_3_no_summary_evidence_{timestamp}"
        run_dir.mkdir(parents=True, exist_ok=True)
        
        print("\n运行 No summary evidence 实验...")
        rows_by_method = self.evaluate_methods({"No summary evidence": self.retrieve_method}, desc="No summary evidence")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="No summary evidence")
    ...
    result = NoSummaryEvidenceExperiment(config).run()
    return 0
