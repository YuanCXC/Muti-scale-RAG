...
Return JSON: {{"correctness": 0-1, "faithfulness": 0-1, "answer_relevance": 0-1, "context_relevance": 0-1}}"""
    ...
    def retrieve_method(self, query: str) -> MethodResult:
        """向量检索"""
        start = time.time()
        units = self.vector_retrieve(query, k=self.config.k3)
        elapsed = (time.time() - start) * 1000
        return MethodResult(units=units, stats={"time_ms": elapsed, "avg_len": context_len(units), "expanded_nodes": 0, "route": "semantic"})
    ...
    def run(self) -> Dict[str, Any]:
        ...
        run_dir = self.config.output_dir / f"exp1_1_semantic_rag_{timestamp}"
        run_dir.mkdir(parents=True, exist_ok=True)
        
        print("\n运行 Semantic RAG 实验...")
        rows_by_method = self.evaluate_methods({"Semantic RAG": self.retrieve_method}, desc="Semantic RAG")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Semantic RAG")
