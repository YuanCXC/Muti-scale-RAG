...
DEFAULT_THRESHOLDS = [0.40, 0.42, 0.43, 0.44, 0.45, 0.46, 0.47, 0.48, 0.50, 0.52]
...
METRIC_LABELS = {
    ...
    "candidate_support_recall": "Candidate Supporting Fact Recall（候选池支撑事实召回率）",
    "selected_support_recall": "Selected Supporting Fact Recall（选择后支撑事实召回率）",
    "candidate_ccr": "Candidate Complete Chain Recall（候选池完整证据链召回率）",
    "final_ccr": "Final Complete Chain Recall（最终完整证据链召回率）",
    "candidate_dccr": "Candidate Document Chain Recall（候选池文档链召回率）",
    "final_dccr": "Final Document Chain Recall（最终文档链召回率）",
    "msfr": "Missing Supporting Fact Recovery（缺失支撑事实恢复率）",
    "msdr": "Missing Supporting Document Recovery（缺失支撑文档恢复率）",
    "no_recovery_rate": "No Recovery Rate（无需恢复比例）",
    "structural_only_rate": "Structural-only Rate（仅结构恢复比例）",
    "bridge_only_rate": "Bridge-only Rate（仅桥接恢复比例）",
    "joint_recovery_rate": "Joint Recovery Rate（联合恢复比例）",
    "structural_activation_rate": "Structural Activation Rate（结构恢复激活率）",
    "bridge_activation_rate": "Bridge Activation Rate（桥接恢复激活率）",
    "average_structural_units": "Average Structural Units（平均结构恢复单元数）",
    "average_bridge_units": "Average Bridge Units（平均桥接恢复单元数）",
...
def _load_split(config: ExperimentConfig, calibration_size: int, seed: int) -> dict:
    split_path = config.work_data_dir / "calibration_split_v4.json"
    split = json.loads(split_path.read_text(encoding="utf-8"))
    if split["calibration_size"] != calibration_size or split["seed"] != seed:
        raise ValueError(
            "The requested calibration size or seed does not match "
            "Calibration Split（固定校准集划分）"
        )
    return split
...
def _metrics_for_result(result, gold_facts: list[tuple[str, int]]) -> dict:
    ...
    selected_facts = [unit.key for unit in result.context_evidence]
    metrics = {
        **supporting_fact_scores(result.supporting_facts, gold_facts),
        **evidence_coverage_scores(candidate_facts, gold_facts, "candidate"),
        **evidence_coverage_scores(selected_facts, gold_facts, "selected"),
        **recovery_scores(
    ...
    metrics["final_ccr"] = metrics["selected_ccr"]
    metrics["final_dccr"] = metrics["selected_dccr"]
    return metrics
...
def _select_threshold(
    ...
    best_threshold = best_f1["structural_threshold"]
    rng = np.random.default_rng(seed)
    assessments = []
    non_inferior = []
    for row in feasible:
        threshold = row["structural_threshold"]
        differences = np.asarray(
            [item["support_f1"] for item in metric_rows[threshold]], dtype=float
        ) - np.asarray(
            [item["support_f1"] for item in metric_rows[best_threshold]], dtype=float
        )
    ...
    return min(
        ...
        key=lambda row: (
            ...
            row["structural_activation_rate"],
            -row["support_f1"],
    ...
    for neighbor_index in sorted(neighbor_indices):
        ...
        for metric in ("support_f1", "final_ccr", "final_dccr"):
            selected_values = np.asarray(
                [row[metric] for row in rows_by_threshold[selected_threshold]],
                dtype=float,
            )
            neighbor_values = np.asarray(
                [row[metric] for row in rows_by_threshold[neighbor]], dtype=float
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Structural Threshold Calibration（结构阈值校准）"
    )
    parser.add_argument(
    ...
    parser.add_argument("--bridge-threshold", type=float, default=0.40)
    ...
    config.bridge_gain_threshold = args.bridge_threshold
    ...
    if args.input_results:
        ...
        output_path = (
            ...
            / f"structural_threshold_calibration_{timestamp}.jsonl"
        )
    ...
    if args.input_results:
        ...
        with output_path.open(encoding="utf-8") as handle:
            for line in handle:
                ...
                threshold = float(detail["structural_threshold"])
        ...
        with output_path.open("w", encoding="utf-8") as handle:
            for example_index, example_id in enumerate(run_ids, start=1):
                ...
                for threshold in thresholds:
                    config.structural_gain_threshold = threshold
    ...
    bootstrap = []
    if selected is not None and args.bootstrap_samples > 0:
    ...
    summary = {
        "experiment": "Structural Threshold Calibration（结构阈值校准实验）",
        ...
        "selection_rule": {
            ...
            "lexicographic_objective": [
                ...
                "Minimize Structural Activation（最小化结构恢复激活率）",
            ],
        },
        "selected_threshold": selected["structural_threshold"] if selected else None,
    ...
    if len(run_ids) == args.calibration_size and selected is not None:
        selection_path = (
            config.output_dir / "structural_threshold_selection_final_v4.json"
        )
        selection_path.write_text(
            json.dumps(
                {
                    "dataset_version": split["dataset_version"],
                    "selection": "Structural Threshold Selection（结构阈值选择结果）",
                    "seed": args.seed,
                    "calibration_size": args.calibration_size,
                    "selected_threshold": selected["structural_threshold"],
                    "bridge_threshold_fixed_at": args.bridge_threshold,
