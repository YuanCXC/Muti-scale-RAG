...
使用 Neo4j 中的结构图谱 + 语义图谱进行固定2-hop扩展检索实验。
...
class TwoHopExpansionExperiment:
    """固定2-hop扩展实验类
    
    使用 Neo4j 图谱进行实体链接和2-hop邻居扩展检索。
    """
    ...
    def two_hop_expansion(self, entity_ids: List[str]) -> Tuple[List[str], Set[str], Dict[str, Tuple[int, float]]]:
        """执行2-hop扩展并按距离和权重排序
        ...
        Returns:
            (排序后的实体ID列表, 扩展后的边ID集合, 实体距离和权重信息)
        """
        entity_info: Dict[str, Tuple[int, float]] = {}
        expanded_edges = set()
        
        for entity_id in entity_ids:
            entity_info[entity_id] = (0, 1.0)
        
        first_hop_info: Dict[str, float] = {}
        ...
        for entity_id in entity_ids:
            ...
            RETURN elementId(neighbor) as neighbor_id, elementId(r) as edge_id, 
            ...
            for record in result:
                ...
                if neighbor_id and neighbor_id not in entity_info:
                    first_hop_info[neighbor_id] = max(
                        first_hop_info.get(neighbor_id, 0),
                        weight
                    )
                if edge_id:
                    expanded_edges.add(edge_id)
        
        for nid, weight in first_hop_info.items():
            entity_info[nid] = (1, weight)
        
        second_hop_info: Dict[str, float] = {}
        
        for entity_id in first_hop_info.keys():
            cypher = """
            MATCH (n)-[r]-(neighbor)
            WHERE elementId(n) = $entity_id
            RETURN elementId(neighbor) as neighbor_id, elementId(r) as edge_id,
            ...
            for record in result:
                ...
                weight = record.get("weight", 1.0)
                if neighbor_id and neighbor_id not in entity_info:
                    second_hop_info[neighbor_id] = max(
                        second_hop_info.get(neighbor_id, 0),
                        weight
                    )
                if edge_id:
                    expanded_edges.add(edge_id)
        
        for nid, weight in second_hop_info.items():
            entity_info[nid] = (2, weight)
        
        sorted_entities = sorted(
            entity_info.keys(),
            key=lambda x: (entity_info[x][0], -entity_info[x][1])
        )
        
        return sorted_entities, expanded_edges, entity_info
    
    def get_entity_titles(self, entity_ids: List[str]) -> List[str]:
        """获取实体对应的标题（保持排序）
        
        Args:
            entity_ids: 实体ID列表（已排序）
        ...
        for entity_id in entity_ids:
            ...
            if result:
                ...
                if node_name and node_name in self.all_titles and node_name not in seen_titles:
                    titles.append(node_name)
                    seen_titles.add(node_name)
    ...
    def generate_answer(self, query: str, retrieved_titles: List[str]) -> str:
        ...
        if not self.llm_client:
            logger.warning("LLM客户端未初始化，无法生成答案")
            return ""
        
        # 构建上下文
        contexts = []
        for title in retrieved_titles[:10]:  # 最多使用前10个文档
            content = self.title_to_content.get(title, "")
            if content:
                contexts.append(f"【{title}】\n{content}")
        
        if not contexts:
            logger.warning("没有找到有效的上下文内容")
            return ""
        
        context_text = "\n\n".join(contexts)
        
        # 构建提示词
        system_prompt = """你是一个专业的问答助手。请根据给定的上下文信息回答用户的问题。
要求：
1. 答案必须基于上下文信息
2. 答案要简洁准确
3. 如果上下文中没有相关信息，请回答"无法从给定信息中找到答案"
4. 不要添加任何额外的解释或说明"""
...
请根据上下文信息回答问题："""
    ...
    def evaluate_single_query(
        ...
        sorted_entities, expanded_edges, entity_info = self.two_hop_expansion(linked_entity_ids)
        
        retrieved_titles = self.get_entity_titles(sorted_entities)
        ...
        title_recall = len(retrieved_titles_set & relevant_titles) / len(relevant_titles) if relevant_titles else 0
        title_precision = len(retrieved_titles_set & relevant_titles) / len(retrieved_titles_set) if retrieved_titles_set else 0
        
        graph_coverage = len(sorted_entities) / self.total_graph_nodes if self.total_graph_nodes > 0 else 0
        
        # 生成答案并计算生成指标
        generation_result = None
        generated_answer = ""
        if ground_truth:
            generated_answer = self.generate_answer(query, retrieved_titles)
            if generated_answer:
                generation_result = self.generation_metrics.compute(
        ...
        return {
            ...
            "expanded_entity_count": len(sorted_entities),
            ...
            "generation_metrics": generation_result.to_dict() if generation_result else None,
        }
    ...
    def run_experiment(
        ...
        if not remaining_indices:
            ...
            for idx in remaining_indices:
                ...
                try:
                    result = self.evaluate_single_query(query, relevant_titles, context_titles, max_k, ground_truth)
        ...
        experiment_result = {
            "experiment_name": "two_hop_expansion",
            "timestamp": datetime.now().isoformat(),
            "config": {
...
def main():
    ...
    experiment = TwoHopExpansionExperiment(
