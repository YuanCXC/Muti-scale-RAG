...
Return JSON: {{"correctness": 0-1, "faithfulness": 0-1, "answer_relevance": 0-1, "context_relevance": 0-1}}"""
    ...
    def retrieve_method(self, query: str) -> MethodResult:
        ...
        return MethodResult(units=units, stats={"time_ms": elapsed, "avg_len": context_len(units), "expanded_nodes": 0, "route": "macrag"})
    ...
    def run(self) -> Dict[str, Any]:
        ...
        run_dir = self.config.output_dir / f"exp1_5_macrag_{timestamp}"
        run_dir.mkdir(parents=True, exist_ok=True)
        
        print("\n运行 MacRAG 实验...")
        rows_by_method = self.evaluate_methods({"MacRAG": self.retrieve_method}, desc="MacRAG")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="MacRAG")
