...
Return JSON: {{"correctness": 0-1, "faithfulness": 0-1, "answer_relevance": 0-1, "context_relevance": 0-1}}"""
    ...
    def retrieve_method(self, query: str) -> MethodResult:
        ...
        reranked = self.rerank_units(query, vec_results)
        
        expanded = []
        if self.graph_store:
            for unit in reranked[:3]:
                try:
                    cypher = f'MATCH (start:Section {{title: "{unit.title}"}})-[:SEMANTIC_LINKS]-(first) MATCH p = (first)-[r*0..2]-(last) MATCH (last)-[:SEMANTIC_LINKS]-(n:Section) WHERE n <> start RETURN DISTINCT n.title AS title, n.sentence_total AS content LIMIT 5'
        ...
        return MethodResult(units=units, stats={"time_ms": elapsed, "avg_len": context_len(units), "expanded_nodes": len(expanded), "route": "fixed_2hop"})
    ...
    def run(self):
        ...
        run_dir = self.config.output_dir / f"exp2_4_fixed_2hop_{timestamp}"
        run_dir.mkdir(parents=True, exist_ok=True)
        
        print("\n运行 Fixed 2-hop 实验...")
        rows_by_method = self.evaluate_methods({"Fixed 2-hop": self.retrieve_method}, desc="Fixed 2-hop")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Fixed 2-hop")
    ...
    result = Fixed2HopExperiment(config).run()
    return 0
