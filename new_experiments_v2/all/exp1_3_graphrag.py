...
Return JSON: {{"correctness": 0-1, "faithfulness": 0-1, "answer_relevance": 0-1, "context_relevance": 0-1}}"""
    ...
    def retrieve_method(self, query: str) -> MethodResult:
        """图扩展"""
        start = time.time()
        initial = self.vector_retrieve(query, k=self.config.k1)
        
        expanded = []
        if self.graph_store:
            for unit in initial[:3]:
                try:
                    cypher = f'MATCH (n:Section {{title: "{unit.title}"}})-[:SEMANTIC_LINKS]-(m:Section) RETURN m.title AS title, m.sentence_total AS content LIMIT 5'
        ...
        all_units = list(initial) + expanded
        units = all_units[:self.config.k3]
        elapsed = (time.time() - start) * 1000
        return MethodResult(units=units, stats={"time_ms": elapsed, "avg_len": context_len(units), "expanded_nodes": len(expanded), "route": "graphrag"})
    ...
    def run(self) -> Dict[str, Any]:
        ...
        run_dir = self.config.output_dir / f"exp1_3_graphrag_{timestamp}"
        run_dir.mkdir(parents=True, exist_ok=True)
        
        print("\n运行 GraphRAG 实验...")
        rows_by_method = self.evaluate_methods({"GraphRAG": self.retrieve_method}, desc="GraphRAG")
