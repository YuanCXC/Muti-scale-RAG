# -*- coding: utf-8 -*-
"""实验3.2：No selective parent

方法：去掉选择性父级回升（总是执行图扩展）
"""
...
class NoSelectiveParentExperiment:
    """No selective parent实验"""
    ...
    def vector_retrieve(self, query: str, granularity: str = "sentence", k: int = 10) -> List[EvidenceUnit]:
...
Return JSON: {{"correctness": 0-1, "faithfulness": 0-1, "answer_relevance": 0-1, "context_relevance": 0-1}}"""
    ...
    def retrieve_method(self, query: str) -> MethodResult:
        """去掉选择性父级回升（总是执行图扩展）"""
        start = time.time()
        vec_results = self.vector_retrieve(query, "sentence", k=self.config.k1)
        kw_results = self.keyword_retrieve(query, k=self.config.k2)
        initial = list(vec_results) + kw_results
        reranked = self.rerank_units(query, initial)
        
        expanded = []
        if self.graph_store:
            for unit in reranked[:3]:
                try:
                    cypher = f'MATCH (start:Section {{title: "{unit.title}"}})-[:SEMANTIC_LINKS]-(first) MATCH p = (first)-[r*0..1]-(last) MATCH (last)-[:SEMANTIC_LINKS]-(n:Section) WHERE n <> start RETURN DISTINCT n.title AS title, n.sentence_total AS content LIMIT 5'
                    rows = self.graph_store.query(cypher)
                    for row in rows:
                        title = row.get("title", "")
                        content = row.get("content", "") or self.title_to_content.get(title, "")
                        if title and content:
                            expanded.append(EvidenceUnit(
                                id=f"graph::{title}",
                                title=title,
                                content=content,
                                score=0.55,
        ...
        all_units = list(reranked) + expanded
        units = all_units[:self.config.k3]
        elapsed = (time.time() - start) * 1000
        return MethodResult(units=units, stats={"time_ms": elapsed, "avg_len": context_len(units), "expanded_nodes": len(expanded), "route": "no_parent"})
    ...
    def run(self) -> Dict[str, Any]:
        ...
        run_dir = self.config.output_dir / f"exp3_2_no_selective_parent_{timestamp}"
        run_dir.mkdir(parents=True, exist_ok=True)
        
        print("\n运行 No selective parent 实验...")
        rows_by_method = self.evaluate_methods({"No selective parent": self.retrieve_method}, desc="No selective parent")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="No selective parent")
    ...
    result = NoSelectiveParentExperiment(config).run()
    return 0
