...
Return JSON: {{"correctness": 0-1, "faithfulness": 0-1, "answer_relevance": 0-1, "context_relevance": 0-1}}"""
    ...
    def retrieve_method(self, query: str) -> MethodResult:
        ...
        units = self.rerank_units(query, vec_results)
        elapsed = (time.time() - start) * 1000
        return MethodResult(units=units, stats={"time_ms": elapsed, "avg_len": context_len(units), "expanded_nodes": 0, "route": "fine_only"})
    ...
    def run(self):
        ...
        run_dir = self.config.output_dir / f"exp2_1_fine_only_{timestamp}"
        run_dir.mkdir(parents=True, exist_ok=True)
        
        print("\n运行 Fine only 实验...")
        rows_by_method = self.evaluate_methods({"Fine only": self.retrieve_method}, desc="Fine only")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Fine only")
    ...
    result = FineOnlyExperiment(config).run()
    return 0
