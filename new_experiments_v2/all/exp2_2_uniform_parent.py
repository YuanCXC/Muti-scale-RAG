...
class UniformParentExperiment:
    ...
    def load_resources(self):
        ...
        self.paragraph_store = FAISSVectorStore(persist_path=str(self.config.paragraph_store_path))
        print("  向量存储: 已加载 (sentence + paragraph)")
    ...
    def vector_retrieve(self, query: str, granularity: str = "sentence", k: int = 10) -> List[EvidenceUnit]:
        """向量检索"""
        store = self.sentence_store if granularity == "sentence" else self.paragraph_store
        if store is None:
            return []
...
Return JSON: {{"correctness": 0-1, "faithfulness": 0-1, "answer_relevance": 0-1, "context_relevance": 0-1}}"""
    ...
    def retrieve_method(self, query: str) -> MethodResult:
        """统一父级回升"""
        start = time.time()
        sentence_results = self.vector_retrieve(query, "sentence", k=self.config.k1)
        paragraph_results = self.vector_retrieve(query, "paragraph", k=5)
        combined = list(sentence_results) + paragraph_results
        units = self.rerank_units(query, combined)
        elapsed = (time.time() - start) * 1000
        return MethodResult(units=units, stats={"time_ms": elapsed, "avg_len": context_len(units), "expanded_nodes": 0, "route": "uniform_parent"})
    ...
    def run(self):
        ...
        run_dir = self.config.output_dir / f"exp2_2_uniform_parent_{timestamp}"
        run_dir.mkdir(parents=True, exist_ok=True)
        
        print("\n运行 Uniform parent 实验...")
        rows_by_method = self.evaluate_methods({"Uniform parent": self.retrieve_method}, desc="Uniform parent")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Uniform parent")
    ...
    result = UniformParentExperiment(config).run()
    return 0
