...
Return JSON: {{"correctness": 0-1, "faithfulness": 0-1, "answer_relevance": 0-1, "context_relevance": 0-1}}"""
    ...
    def retrieve_method(self, query: str) -> MethodResult:
        ...
        return MethodResult(units=units, stats={"time_ms": elapsed, "avg_len": context_len(units), "expanded_nodes": len(expanded), "route": "fixed_1hop"})
    ...
    def run(self):
        ...
        run_dir = self.config.output_dir / f"exp2_3_fixed_1hop_{timestamp}"
        run_dir.mkdir(parents=True, exist_ok=True)
        
        print("\n运行 Fixed 1-hop 实验...")
        rows_by_method = self.evaluate_methods({"Fixed 1-hop": self.retrieve_method}, desc="Fixed 1-hop")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Fixed 1-hop")
    ...
    result = Fixed1HopExperiment(config).run()
    return 0
