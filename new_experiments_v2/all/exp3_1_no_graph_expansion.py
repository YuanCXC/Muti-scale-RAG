...
class NoGraphExpansionExperiment:
    """No graph expansion实验"""
...
Return JSON: {{"correctness": 0-1, "faithfulness": 0-1, "answer_relevance": 0-1, "context_relevance": 0-1}}"""
    ...
    def retrieve_method(self, query: str) -> MethodResult:
        ...
        return MethodResult(units=units, stats={"time_ms": elapsed, "avg_len": context_len(units), "expanded_nodes": 0, "route": "no_graph"})
    ...
    def run(self) -> Dict[str, Any]:
        ...
        run_dir = self.config.output_dir / f"exp3_1_no_graph_expansion_{timestamp}"
        run_dir.mkdir(parents=True, exist_ok=True)
        
        print("\n运行 No graph expansion 实验...")
        rows_by_method = self.evaluate_methods({"No graph expansion": self.retrieve_method}, desc="No graph expansion")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="No graph expansion")
    ...
    result = NoGraphExpansionExperiment(config).run()
    return 0
