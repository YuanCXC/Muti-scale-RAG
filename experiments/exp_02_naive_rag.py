# -*- coding: utf-8 -*-
"""实验02: 纯向量检索 RAG (Naive RAG)

使用向量检索获取 Top-K 文档，然后 LLM 生成答案。
这是最基础的 RAG 实现。
...
from src.chains import NaiveRAGChain
...
def build_vector_index(
    ...
    print(f"向量索引构建完成: {vector_store.count()} 个文档")
    
    return vector_store, doc_map
...
def run_naive_rag_experiment(
    ...
    """
    print(f"\n{'='*60}")
    print(f"实验02: 纯向量检索 RAG (Naive RAG)")
    print(f"描述: 使用向量检索获取 Top-K 文档，然后 LLM 生成答案")
    print(f"{'='*60}\n")
    ...
    vector_store, doc_map = build_vector_index(
        knowledge_base, 
        embedding_client,
...
def _run_with_evaluator(
    ...
    evaluator = RAGEvaluator(
        workflow=chain,
        workflow_type=WorkflowType.NAIVE,
...
def _run_manual(
    ...
    report = {
        "experiment_name": "纯向量检索 RAG",
        "experiment_id": config.experiment_id,
        "description": "使用向量检索获取 Top-K 文档，然后 LLM 生成答案",
...
def main():
    ...
    report = run_naive_rag_experiment(config, max_samples=10, use_evaluator=True)
