...
基于 My_project 思路的改进版自适应多阶段检索策略。
...
class AdaptiveStrategyExperiment:
    """自适应策略实验类
    
    结合向量检索、图谱扩展、重排序和LLM打分的多阶段检索策略。
    """
...
关键词:"""
    ...
    def step3_keyword_retrieval(self, keywords: List[str]) -> List[Dict]:
        ...
        def extract_node_property(node, prop_name, default=""):
            """安全提取节点属性，支持 dict 和 Neo4j Node 对象"""
            if node is None:
                return default
...
答案:"""
    ...
    def _load_checkpoint(self) -> Optional[Dict]:
        ...
        try:
            ...
            if "aggregated_metrics" in checkpoint:
                ...
                # 只有当值是字典类型时才进行键转换
                if "recall_at_k" in agg_metrics and isinstance(agg_metrics["recall_at_k"], dict):
                    agg_metrics["recall_at_k"] = {int(k): v for k, v in agg_metrics["recall_at_k"].items()}
                if "precision_at_k" in agg_metrics and isinstance(agg_metrics["precision_at_k"], dict):
    ...
    def run_experiment(
        ...
        baseline_recall = 0.1
        recall_improvement = (final_metrics["recall_at_k"] - baseline_recall) / baseline_recall * 100 if baseline_recall > 0 else 0
        final_metrics["recall_improvement"] = recall_improvement
        
        experiment_result = {
            "experiment_name": "adaptive_strategy_improved",
            "timestamp": datetime.now().isoformat(),
            "config": {
    ...
    def _save_results(self, results: Dict[str, Any], save_details: bool) -> None:
        ...
        metrics_rows = [
            {
                "Metric": f"Recall@{config['k4']}",
                "Value": metrics["recall_at_k"],
            },
            {
                "Metric": f"Precision@{config['k4']}",
                "Value": metrics["precision_at_k"],
            },
            {
                "Metric": "Recall提升率(%)",
                "Value": metrics["recall_improvement"],
            },
            {
                "Metric": "MRR",
                "Value": metrics["mrr"],
    ...
    def _print_results(self, results: Dict[str, Any]) -> None:
        ...
        print(f"  向量存储: {list(config['vector_store_paths'].keys())}")
        ...
        print(f"\n【Recall@{config['k4']}】")
        print(f"  Recall@{config['k4']}: {metrics['recall_at_k']:.4f}")
        
        print(f"\n【Precision@{config['k4']}】")
        print(f"  Precision@{config['k4']}: {metrics['precision_at_k']:.4f}")
        
        print("\n【Recall提升率】")
        print(f"  Recall@{config['k4']} 提升: {metrics['recall_improvement']:.2f}%")
        ...
        print("\n【延迟指标】")
        print(f"  平均向量检索延迟: {metrics['avg_vector_latency']:.4f}s")
        print(f"  平均图谱检索延迟: {metrics['avg_graph_latency']:.4f}s")
...
def main():
    ...
    output_dir = "e:/Code_Personal/Subject/test02/experiments/exp6_adaptive_strategy_improved"
    
    experiment = AdaptiveStrategyExperiment(
