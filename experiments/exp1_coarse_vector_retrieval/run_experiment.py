...
class CoarseVectorRetrievalExperiment:
    """粗粒度向量检索实验类"""
    ...
    def generate_answer(self, query: str, retrieved_titles: List[str]) -> str:
        ...
        """
        context_parts = []
        for i, title in enumerate(retrieved_titles[:3]):
            content = self.title_to_content.get(title, title)
...
答案:"""
    ...
    def evaluate_single_query(
        ...
        title_recall = len(seen_titles & relevant_titles) / len(relevant_titles) if relevant_titles else 0
        title_precision = len(seen_titles & relevant_titles) / len(seen_titles) if seen_titles else 0
        
        generated_answer = self.generate_answer(query, retrieved_titles[:5])
    ...
    def run_experiment(
        ...
        experiment_result = {
            "experiment_name": "coarse_vector_retrieval",
            "timestamp": datetime.now().isoformat(),
            "config": {
...
def main():
    """主函数"""
    vector_store_path = "e:/Code_Personal/Subject/test02/data/hotpotqa/vector_stores/valid_title_sentence"
    test_data_path = "e:/Code_Personal/Subject/test02/data/hotpotqa/validation-00000-of-00001.parquet"
    output_dir = "e:/Code_Personal/Subject/test02/experiments/exp1_coarse_vector_retrieval"
    
    experiment = CoarseVectorRetrievalExperiment(
