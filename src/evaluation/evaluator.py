...
class RAGEvaluator:
    ...
    用于评估 RAG 系统的性能，主要使用 LLM-based 指标。
    """
    ...
    def __init__(
        ...
        self.generation_metrics = GenerationMetrics(k_values=self.k_values)
        self.system_metrics = SystemMetrics()
        
        logger.debug(f"初始化 RAGEvaluator: workflow_type={workflow_type.value}")
    ...
    def evaluate(
        ...
        Returns:
            EvaluationReport 实例
        """
        logger.debug(f"开始评估 {len(samples)} 个样本")
        ...
        if resume and self.checkpoint_path:
            ...
            logger.debug(f"从检查点恢复: start_idx={start_idx}")
        ...
        for i, sample in enumerate(tqdm(samples[start_idx:], desc="评估中"), start=start_idx):
            try:
                ...
                if chain_result.success:
                    ...
                    retrieved_contexts = [s.get("content", "") for s in chain_result.sources if s.get("content")]
                    ...
                    gen_metrics = self.generation_metrics.compute(
                        ...
                        compute_llm_based=self.llm_client is not None,
    ...
    def _aggregate_metrics(self, metrics_list: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for key in keys:
            ...
            if values:
                ...
                k_dicts = [m.get(key, {}) for m in metrics_list if isinstance(m.get(key), dict)]
                if k_dicts:
                    all_k_keys = set()
                    for kd in k_dicts:
                        all_k_keys.update(kd.keys())
                    aggregated[key] = {}
                    for k_key in all_k_keys:
                        k_values = [kd.get(k_key, 0) for kd in k_dicts if isinstance(kd.get(k_key), (int, float))]
                        if k_values:
                            aggregated[key][k_key] = sum(k_values) / len(k_values)
    ...
    def save_report(self, report: EvaluationReport, output_path: str) -> None:
        ...
        logger.debug(f"报告已保存到: {output_path}_report.json")
